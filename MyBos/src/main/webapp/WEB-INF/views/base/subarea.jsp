...
<script type="text/javascript">
	...
	function doEdit(){
		...
		if (rows.length!=1){
			$.messager.alert("提示","请正确选中要修改的条目！","warning")
		}else{
	...
	function doDelete(){
		...
		if (rows.length==0){
			$.messager.alert("提示","请选中要删除的条目！","warning")
		}else{
			$.messager.confirm("确认","确认要删除选中的条目吗？",function(r){
				if (r){					
					var array = new Array();				
					for (var i =0 ; i<rows.length; i++){						
						array.push(rows[i].id);
					}
					location.href="subareaAction_deleteSubarea.action?ids="+array.join(",");
				}				
			});
	...
	$(function(){
		...
		$("#btn").click(function(){
			...
			console.info(p);//在控制台打印p对象，此处为js代码，使用alert(p)无法看到对象内容
	...
	<div class="easyui-window" title="分区添加修改" id="addSubareaWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="height:31px;overflow:hidden;" split="false" border="false" >
			<div class="datagrid-toolbar">
				<a id="save" onclick="saveOnClick()" icon="icon-save" href="#" class="easyui-linkbutton" plain="true" >保存</a>
			</div>
		</div>
...
<!-- 					<tr>
	...
	<div class="easyui-window" title="分区添加修改" id="editSubareaWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="height:31px;overflow:hidden;" split="false" border="false" >
			<div class="datagrid-toolbar">
				<a id="save" onclick="saveOnClickEdit()" icon="icon-save" href="#" class="easyui-linkbutton" plain="true" >保存</a>
			</div>
		</div>
